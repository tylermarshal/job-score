...
describe 'a logged in user visits the resume anlaysis page' do
  it 'the user can see their resume analysis' do
    user = create(:user)

    resume_text = "As a customer-obsessed professional with a passion for the healthcare industry, I was thrilled to learn that Health First Medical Clinic needs a Customer Service Manager. Presently, I am a Senior Customer Service Representative with six years of experience. I am registered with the California State Board of Pharmacy and have a B.A. in Management from Golden Gate University in San Francisco.
...
You know how and when to escalate when you are stuck on a task"
    ...
    resume = create(:resume, body: resume_text, user: user)

    allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(user)

    visit resume_analysis_path

    expect(page).to have_content("Resume Analysis")
    expect(page).to have_content("Keywords")
    expect(page).to have_content("Word Count")
    expect(page).to have_content("Your resume word count is low and you should consider adding more content. Summaries, projects, volunteer work, skills, and awards are great additions beyond work experience and education.")
