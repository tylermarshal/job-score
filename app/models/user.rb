class User < ApplicationRecord
  ...
  def from_omniauth(auth_info)
    ...
    if user.nil?
      create do |new_user|
        create_or_update(new_user, auth_info)
      end
    else
      if user.created_at + user.expires <= Time.now
        user
      else
        create_or_update(user, auth_info)
        user.save!
      end
    end
  end

  def create_or_update(user_type, auth_info)
    ...
    user_type.auth_expires       = auth_info.extra.access_token.params.oauth_authorization_expires_in
  end
end
